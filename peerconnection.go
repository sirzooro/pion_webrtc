...
func (pc *PeerConnection) SetLocalDescription(desc SessionDescription) error {
	...
	// packets from the peer aren't dropped.
	pc.mu.Lock()
	...
	} else if err := pc.dtlsTransport.updateCryptexModes(
		pc.dtlsTransport.getLocalCryptexMode(), newRemoteCryptexMode,
	); err != nil {
		return &rtcerr.InvalidModificationError{Err: err}
	}
...
func (pc *PeerConnection) SetRemoteDescription(desc SessionDescription) error {
	...
	// Only apply the negotiated Cryptex modes to the already-started SRTP session once the
	// description above is actually accepted, so a rejected signaling transition doesn't leave
	// the live SRTP session mutated for a description that was never committed.
	if isRenegotiation {
		if err := pc.dtlsTransport.updateCryptexModes(localCryptexMode, remoteCryptexMode); err != nil {
			return &rtcerr.InvalidModificationError{Err: err}
		}
	}
...
func (pc *PeerConnection) cryptexModeFromSDP(parsed *sdp.SessionDescription) (srtp.CryptexMode, error) {
	...
	// When the SDP contains no active RTP media (for example, an initial datachannel-only
	// negotiation), we still need to select a stable Cryptex mode for the DTLS transport.
	// Otherwise, when RTP m-lines are added later via renegotiation, we would attempt to
	// change Cryptex mode after SRTP has already been created.
	noActiveRTPMedia := !negotiatedForAnyMedia && negotiatedForAllMedia
	if noActiveRTPMedia {
		switch pc.configuration.RTPHeaderEncryptionPolicy {
		case RTPHeaderEncryptionPolicyDisable:
			return srtp.CryptexModeDisabled, nil
		case RTPHeaderEncryptionPolicyRequire:
			return srtp.CryptexModeRequired, nil
		case RTPHeaderEncryptionPolicyUnknown, RTPHeaderEncryptionPolicyNegotiate:
			return srtp.CryptexModeEnabled, nil
		}
	}
