...
func (pc *PeerConnection) SetRemoteDescription(desc SessionDescription) error {
	...
	cryptexMode, cryptexErr := pc.getCryptexModeFromRemoteDescription(desc.parsed)
	pc.mu.Unlock()
	if cryptexErr != nil {
		return cryptexErr
	}
...
// getCryptexModeFromRemoteDescription requires the caller holds pc.mu.
func (pc *PeerConnection) getCryptexModeFromRemoteDescription(parsed *sdp.SessionDescription) (srtpCryptexMode, error) {
	negotiatedForAnyMedia, negotiatedForAllMedia := cryptexNegotiatedInSDP(parsed)
	var mode srtpCryptexMode

	switch pc.configuration.RTPHeaderEncryptionPolicy {
	case RTPHeaderEncryptionPolicyDisable:
		mode = srtpCryptexModeDisabled
	case RTPHeaderEncryptionPolicyUnknown, RTPHeaderEncryptionPolicyNegotiate:
		if negotiatedForAnyMedia {
			mode = srtpCryptexModeEnabled
		} else {
			mode = srtpCryptexModeDisabled
		}
	case RTPHeaderEncryptionPolicyRequire:
		if !negotiatedForAllMedia {
			return srtpCryptexModeDisabled, &rtcerr.InvalidAccessError{Err: ErrRTPHeaderEncryptionRequired}
		}
		mode = srtpCryptexModeRequired
	}
