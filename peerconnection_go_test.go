...
func dtlsCryptexMode(pc *PeerConnection) srtpCryptexMode {
	...
	for _, policy := range tests {
		t.Run(policy.String(), func(t *testing.T) {
			...
			case RTPHeaderEncryptionPolicyUnknown, RTPHeaderEncryptionPolicyNegotiate, RTPHeaderEncryptionPolicyRequire:
				...
				assert.True(t, hasApplicationCryptex, "expected a=cryptex on offer application m-line")
			default:
	...
	for _, policy := range tests {
		t.Run(policy.String(), func(t *testing.T) {
			...
			if policy == RTPHeaderEncryptionPolicyDisable {
				assert.Equal(t, srtpCryptexModeDisabled, dtlsCryptexMode(pcOffer))
				assert.Equal(t, srtpCryptexModeDisabled, dtlsCryptexMode(pcAnswer))
				...
				assert.Equal(t, srtpCryptexModeEnabled, dtlsCryptexMode(pcOffer))
				if policy == RTPHeaderEncryptionPolicyRequire {
					assert.Equal(t, srtpCryptexModeRequired, dtlsCryptexMode(pcAnswer))
				} else {
					assert.Equal(t, srtpCryptexModeEnabled, dtlsCryptexMode(pcAnswer))
				}
